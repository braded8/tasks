...
export function isCorrect(question: Question, answer: string): boolean {
    const fixAnswer = answer.trim().toLowerCase();
    const fixQuestion = {
        ...question,
        expected: question.expected.trim().toLowerCase()
    };
    const result = fixAnswer === fixQuestion.expected;
    return result;
}
...
export function isValid(question: Question, answer: string): boolean {
    let result: boolean = question.type === "short_answer_question";
    result
        ? result
...
export function toShortForm(question: Question): string {
    ...
    question.name.length > 10
        ? (copy = question.name.slice(0, 10))
        : (copy = question.name);
    return String(question.id) + ": " + copy;
}
...
export function toMarkdown(question: Question): string {
    ...
    const hashtag = "# ";
    const newline = "\n";
    let markdown: string = hashtag.concat(
        question.name,
        newline,
        question.body
    );
